            ...
            <div style={{ paddingLeft: width + "px", height: "calc(100vh - 50px)" }} className={isOpen && isMdScreen ? 'absolute' : ''}>
                <div
                    ...
                    <div className="flex flex-col gap-1 w-0 pt-[4.3rem] h-fit" style={{ marginRight: "40px" }}>
                        {sidebarPlugins.map(({ name, iconUrl, pluginId, actionKey }, index) => {
                            const plugin = plugins.find(p => p.id === pluginId)!;
                            return (
                                <button
                                    key={name}
                                    style={{ background: index === openPlugin ? "rgb(94, 102, 115)" : "rgb(44, 52, 65)" }}
                                    onClick={() => {
                                        setOpenPlugin(index === openPlugin ? -1 : index);
                                        setSidebarPlugin(plugin);
                                        setPluginAction(getContextMenuAction(pluginId, actionKey, ""));
                                    }}
                                    className="flex flex-col items-center rounded-l-lg py-3 brightness-200 dark:brightness-100 w-[40px]">
                                    <img src={iconUrl} className="w-6 h-6 brightness-75" title={`${plugin.title} - ${name}`} />
                                </button>
                            );
                        })}
                    </div>
